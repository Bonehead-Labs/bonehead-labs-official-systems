...
func _ensure_theme_service() -> bool:
    if _theme_service() == null:
        if not _missing_theme_logged:
            ...
            push_error("BaseToggle: ThemeService autoload not found. Add ThemeService before instantiating BaseToggle.")
        return false
    return true
...
func _report_missing_localization() -> void:
    ...
    push_error("BaseToggle: ThemeLocalization autoload not found. Token translation is unavailable.")
