...
@export var sections_container_path: NodePath = NodePath("Layout/Sections/SectionList")
@export var footer_container_path: NodePath = NodePath("Layout/Footer/ActionBar")
...
func _create_section(config: Variant) -> Node:
	...
	if data.has(KEY_DESCRIPTION):
		...
		var description_text: Variant = data[KEY_DESCRIPTION]
...
func _create_toggle(data: Dictionary) -> Control:
	...
	toggle.text = resolve_text(text_value)
...
func _create_slider(data: Dictionary) -> Control:
	print("SettingsTemplate: Creating slider with data: ", data)
	...
	var resolved_text: String = resolve_text(label_text)
	if resolved_text != "":
		var label: Label = _WidgetFactory.create_label({})
		label.text = resolved_text
		label.size_flags_horizontal = Control.SIZE_EXPAND_FILL
		container.add_child(label)
	
	# Create the slider
	var slider: HSlider = _WidgetFactory.create_slider({})
	print("SettingsTemplate: Slider created: ", slider)
	
	# Set proper sizing for the slider
	...
	print("SettingsTemplate: Slider config: ", slider_config)
	_UITemplateDataBinder.apply_slider_value(slider, slider_config)
	print("SettingsTemplate: Slider value applied, current value: ", slider.value)
	...
	print("SettingsTemplate: Slider setup complete, container size: ", container.size, " slider size: ", slider.size)
	return container
