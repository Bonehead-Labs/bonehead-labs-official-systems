...
func get_font_size(token: StringName) -> int:
	var sizes_raw = typography.get(StringName("sizes"), {})
	var sizes: Dictionary[StringName, int] = {}
	if sizes_raw is Dictionary:
		for key in sizes_raw.keys():
			if key is StringName and sizes_raw[key] is int:
				sizes[key] = sizes_raw[key]
